...
# Choropleth of percent below federal poverty limit

```{r}
poverty_plot <- dc_indicators |>
	ggplot() +
	geom_sf(aes(fill = federal_poverty_limit_below_allraces_percent)) +
	...
	labs(
		title = "% below the federal poverty limit",
		caption = "ACS 5-year (2024 vintage) by census tract",
		x = NULL,
		y = NULL
	) +
	theme_sub_panel(
	...
	theme_sub_axis(text = element_blank()) +
	theme_sub_legend(title = element_blank())

rlsave(
	filename = "outputs/dc-federal-poverty-2024.png",
	plot = poverty_plot
)
